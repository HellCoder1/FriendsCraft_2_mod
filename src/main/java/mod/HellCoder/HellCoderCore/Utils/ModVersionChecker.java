...
public class ModVersionChecker
{
    ...
    public ModVersionChecker(String modID, String curVer, String versionURL, String updateURL, String[] loadMsg, String[] inGameMsg, int timeoutMS)
    {
        ...
        try
        {
            ...
            FCLog.info("Initializing ModVersionChecker for mod %s", modID);
        }
        catch (Throwable e)
        {
        	FCLog.warning("Error initializing ModVersionChecker for mod %s: %s", modID, e.getMessage());
        }
        
        String[] versionLines = CommonUtils.loadTextFromURL(this.versionURL, FCLog.INSTANCE.getLogger(), new String[] { CHECK_ERROR }, timeoutMS);
