...
public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer)
{
  ...
  renderer.setRenderBoundsFromBlock(block);
   drawFaces(renderer, block, ((mod.HellCoder.things.Blocks.DigaOre)block).icon[0], false);
   Color c = new Color(16777215);
   float r = c.getRed() / 255.0F;
   float g = c.getGreen() / 255.0F;
   float b = c.getBlue() / 255.0F;
   GL11.glColor3f(r, g, b);
   drawFaces(renderer, block, ((mod.HellCoder.things.Blocks.DigaOre)block).icon[1], false);
...
public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer)
{
  ...
  renderer.renderStandardBlock(block, x, y, z);
    ...
    renderAllSides(world, x, y, z, block, renderer, ((mod.HellCoder.things.Blocks.DigaOre)block).icon[1], false);
